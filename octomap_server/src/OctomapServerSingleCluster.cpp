...
#include <octomap_server/OctomapServerSingleCluster.h>
...
OctomapServerSingleCluster::OctomapServerSingleCluster(ros::NodeHandle private_nh_)
: m_nh(),
  ...
  m_incrementalUpdate(false)
{
  ...
  m_tfPointCloudSub->registerCallback(boost::bind(&OctomapServerSingleCluster::insertCloudCallback, this, _1));
  ...
  m_clearBBXService = private_nh.advertiseService("clear_bbx", &OctomapServerSingleCluster::clearBBXSrv, this);
  m_resetService = private_nh.advertiseService("reset", &OctomapServerSingleCluster::resetSrv, this);

  dynamic_reconfigure::Server<OctomapServerConfig>::CallbackType f;

  f = boost::bind(&OctomapServerSingleCluster::reconfigureCallback, this, _1, _2);
  m_reconfigureServer.setCallback(f);
}

OctomapServerSingleCluster::~OctomapServerSingleCluster(){
...
bool OctomapServerSingleCluster::openFile(const std::string& filename){
...
bool OctomapServerSingleCluster::octomapClusterSrv(OctomapSingleCluster::Request  &req, OctomapSingleCluster::Response &res){
  ...
  if (m_filterGroundPlane){
    ...
    try{
      m_tfListener.waitForTransform(m_baseFrameId, req.cluster.header.frame_id, req.cluster.header.stamp, ros::Duration(0.2));
      m_tfListener.lookupTransform(m_baseFrameId, req.cluster.header.frame_id, req.cluster.header.stamp, sensorToBaseTf);
      m_tfListener.lookupTransform(m_worldFrameId, m_baseFrameId, req.cluster.header.stamp, baseToWorldTf);
  ...
  publishCluster(res, req.cluster.header.stamp);
  return true;

}

void OctomapServerSingleCluster::insertCloudCallback(const sensor_msgs::PointCloud2::ConstPtr& cloud){
  ros::WallTime startTime = ros::WallTime::now();
...
void OctomapServerSingleCluster::insertScan(const tf::Point& sensorOriginTf, const PCLPointCloud& ground, const PCLPointCloud& nonground){
...
void OctomapServerSingleCluster::publishAll(const ros::Time& rostime){
  ros::WallTime startTime = ros::WallTime::now();
...
void OctomapServerSingleCluster::publishCluster(OctomapSingleCluster::Response &response, const ros::Time& rostime){
  ros::WallTime startTime = ros::WallTime::now();
  size_t octomapSize = m_octree->size();
  // TODO: estimate num occ. voxels for size of arrays (reserve)
  if (octomapSize <= 1){
...
bool OctomapServerSingleCluster::octomapBinarySrv(OctomapSrv::Request  &req,
...
bool OctomapServerSingleCluster::octomapFullSrv(OctomapSrv::Request  &req,
                                    OctomapSrv::Response &res)
{
...
bool OctomapServerSingleCluster::clearBBXSrv(BBXSrv::Request& req, BBXSrv::Response& resp){
...
bool OctomapServerSingleCluster::resetSrv(std_srvs::Empty::Request& req, std_srvs::Empty::Response& resp) {
  visualization_msgs::MarkerArray occupiedNodesVis;
  occupiedNodesVis.markers.resize(m_treeDepth +1);
...
void OctomapServerSingleCluster::publishBinaryOctoMap(const ros::Time& rostime) const{
...
void OctomapServerSingleCluster::publishFullOctoMap(const ros::Time& rostime) const{
...
void OctomapServerSingleCluster::filterGroundPlane(const PCLPointCloud& pc, PCLPointCloud& ground, PCLPointCloud& nonground) const{
...
void OctomapServerSingleCluster::handlePreNodeTraversal(const ros::Time& rostime){
  if (m_publish2DMap){
...
void OctomapServerSingleCluster::handlePostNodeTraversal(const ros::Time& rostime){

  if (m_publish2DMap)
    m_mapPub.publish(m_gridmap);
}

void OctomapServerSingleCluster::handleOccupiedNode(const OcTreeT::iterator& it){
...
void OctomapServerSingleCluster::handleFreeNode(const OcTreeT::iterator& it){
...
void OctomapServerSingleCluster::handleOccupiedNodeInBBX(const OcTreeT::iterator& it){
...
void OctomapServerSingleCluster::handleFreeNodeInBBX(const OcTreeT::iterator& it){
...
void OctomapServerSingleCluster::update2DMap(const OcTreeT::iterator& it, bool occupied){
...
bool OctomapServerSingleCluster::isSpeckleNode(const OcTreeKey&nKey) const {
  OcTreeKey key;
...
void OctomapServerSingleCluster::reconfigureCallback(octomap_server::OctomapServerConfig& config, uint32_t level){
...
void OctomapServerSingleCluster::adjustMapData(nav_msgs::OccupancyGrid& map, const nav_msgs::MapMetaData& oldMapInfo) const{
...
std_msgs::ColorRGBA OctomapServerSingleCluster::heightMapColor(double h) {
